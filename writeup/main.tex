...
Over the last two decades there have been two dominant grammatical
frameworks for statistical syntactic parsing: phrase-structure and
dependency parsing \cite{}. The two frameworks often offer a practical
...
To approach this question we present a very simple statistical parser,
\textsc{Miniparser}.  The parser is a complete exact
...
with all non-preterminal rules of the form $\langle A \rightarrow \beta_1 \beta_2 \rangle$. 
...
\[ \infer{(\Span{i, j}, h, A)}{(\Span{i, k}, m, \beta_1) &  (\Span{k+1, j}, h, \beta_2)} \]
 for all rules $\langle A \rightarrow \beta_1\ \beta_2 , 1\rangle\in \rules$ and spans $i \leq k < j$. This rule indicates that grammatical rule $\langle A \rightarrow \beta_1\ \beta_2, 1\rangle$  was applied at a vertex covering $\Span{i, j}$ to produce two vertices covering $\Span{i, k}$ and $\Span{k+1, j}$.
...
\begin{figure}
  ...
  \noindent\textbf{Rules:}
   ...
   For all   $i < m, h = d_m$  and rule  $\langle A \rightarrow \beta_1\ \beta_2, 1 \rangle$,
  \[\infer{( \Span{ i, \Right{m} }, h, A)}{(\Span{i, \Left{m}-1}, h, \beta_1)  &  (\tri, m, \beta_2) } \]

  For all    $m < j, h = d_m$ and  rule  $\langle A \rightarrow \beta_1\ \beta_2, 2\rangle$,
...
if we use the label set ${\cal L} = \nonterms \times \nonterms \times \nonterms$, encoding the binary rule decisions $A \rightarrow \beta_1\ \beta_2$.
...
\item
$\langle A^{\Rightarrow}_{\beta_i}  \rightarrow  A^{\Rightarrow}_{\beta_{i-1}} \ \beta_i, 1 \rangle $ for $k > i > m$

\item
$\langle A^{\Leftarrow}_{\beta_i}  \rightarrow \beta_i\ A^{\Leftarrow}_{\beta_{i+1}}, 2 \rangle $ for $1< i < k$
\end{itemize}  
...
\item $\langle A^{\Rightarrow}_{\beta_{k+1}} \rightarrow \beta_k \  \beta_{k+1}, 1 \rangle $

\item 
$\langle A^{\Rightarrow}_{\END}  \rightarrow  A^{\Rightarrow}_{\beta_{m-1}} \ \beta_m, 1 \rangle $

\item  $\langle A^{\Leftarrow}_{\beta_{k-1}} \rightarrow \beta_{k-1}\  A^{\Rightarrow}_{\END}, 2 \rangle $

\item
$\langle A  \rightarrow \beta_1\ A^{\Leftarrow}_{\beta_{2}}, 2 \rangle $ 
\end{itemize}
...
The first set of features look at the rule structure $A \rightarrow \beta_1\ \beta_2$.
...
\begin{figure}
  \centering
  For a part $(\span{i, j, k}, h, m, A \rightarrow \beta_1\ \beta_2)$
  ...
  $(A \rightarrow \beta_1\ \beta_2, \TagFN{h}, \TagFN{m})$ \\
  $(A \rightarrow \beta_1\ \beta_2, \WordFN{h}, \TagFN{m})$ \\
  $(A \rightarrow \beta_1\ \beta_2, \WordFN{h})$ \\
  $(A \rightarrow \beta_1\ \beta_2, \TagFN{h})$ \\
  $(A \rightarrow \beta_1\ \beta_2, \WordFN{m})$ \\
  $(A \rightarrow \beta_1\ \beta_2, \TagFN{m})$ \\
