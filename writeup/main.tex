...
provides a different representations of a sentence's structure \cite{collins2003head,de2008stanford,yamada2003statistical,johansson2007extended}.
...
\scalebox{0.7}{\Tree [ .X(2) [ .X(1) [ .N I$_1$ ] ]  [ .V saw$_2$ ] [ .X(4) [ .D
    the$_3$ ]  [ .N man$_4$ ] ] ]
 \Tree [ .X(2)  [ .N I$_1$ ] [ .X(2)  [ .V saw$_2$ ] [ .X(4) [ .D
    the$_3$ ]  [ .N man$_4$ ] ] ] ] } \\

\scalebox{0.7}{    \Tree [ .X(2) [ .X(2) [ .N I$_1$ ]   [ .V saw$_2$ ] ]  [ .X(4) [
    .D the$_3$ ]  [ .N man$_4$ ] ] ] }
...
\caption{The two algorithms written as deductive parsers. Starting
  ...
  parse. Finding the optimal c-parse with the new algorithm is  $O\left((\sum_{h} L(h) \times R(h)) |\rules|\right)$ where $L$ and $R$ are the 
...
For each word $x_h$ in a d-parse, define $L(h)$ and $R(h)$ as the
...
orderings of these combinations for each head. Since any rule in
$\rules$ can be used at each point, the running time of the algorithm
is $O((\sum_{h} L(h) \times R(h)) |\rules|)$.  While the number of
dependents $L$ and $H$ could in theory be $O(n)$, in practice the
number is often a constant term as we will show in Figure~\ref{fig:speed}. 
...
exploit this property we build tables $\rules_{\TagFN{h}}$ for each
part-of-speech tag and limit the search to rules seen for the
head tag. This further reduces the run-time of the 
algorithm to $O(\sum_{h} (L(h) \times R(h) \times |\rules_{\TagFN{h}}|))$.


Second, as noted above, at any point a word $x_h$ with may combine with its
next left or right dependent, leading to $L(h)\times R(h)$ possible orderings.
However, in practice we can often predict which side will come next. 
To make this prediction, we estimate a distribution 

\[\eta(\textrm{side}) = p(\textrm{side} \mid \TagFN{x_h}, \TagFN{x_l}, \TagFN{x_r})\]

where $x_l$ is the next left dependent, $x_r$ is the next right
dependent, and $\TagFN{}$ is the part-of-speech tag of the word. If
the conditional probability of $\eta{\mathrm{left}}$ or
$\eta{\mathrm{right}}$ is greater than a threshold parameter $\gamma$,
we make a hard decision to combine with that side next. This pruning
further reduces the impact of outlier words $h$ with large $L(h)
\times R(h)$.
...
\begin{table}
  ...
  \begin{tabular}{|l|crr|}
    ...
    \hline
    \hline
    \textsc{Full[$< 20$]} & $O(n^5|\rules|)$ &  0.25  & 100.0  \\
    \textsc{Dep}          & $O(n^2|\rules|)$ &  76.2  & 92.9    \\
    \textsc{Pruned (1)}   & $O(n^2|\rules|)$ &  375.9 & 92.8    \\
    \textsc{Pruned (1\&2)}   & $O(n^2|\rules|)$ &  487.3 & 92.6 \\
    \hline
  \end{tabular}
  \label{tab:alg-oracle}
  \caption{Comparison of three parsing setups: 
    \textsc{Full[$<20$]} is the standard lexicalized grammar,
    limited for efficiency to sentence of length less than 20 words,
    \textsc{Dep} restricts c-parses to be consistent with a
    predicted dependency skeleton, and \textsc{Pruned(1)} and \textsc{Pruned(2)}  is the pruning described in
    Section~\ref{sec:prune} \smrcomment{regen this table}. The oracle is the best labeled $F_1$ achievable on the
...
\item binned length of the span
\end{itemize}
...
results using \S 19 and the full training set. \nascomment{check that
  ``\S 2--21'' is correct, pls} \lpkcomment{you mean our number or you mean the standard training splits?}
results using \S 19 and the full training set. 
  \label{tab:convert}}
\end{table}
...
and \nascomment{last one seems to be missing}
 the effect of the amount of annotated phrase-structure
data.
...
\hline
  ...
  \caption{ Accuracy and speed on PTB \S 23 \nascomment{check!  said
      CTB as well}. and CTB 5.1 test split. Comparisons are to state-of-the-art non-reranking supervised
...
accuracy is the that of \newcite{zhu2013fast}, a fast shift-reduce phrase-structure parser.

\nascomment{what about Chinese??}
To apply our model on Chinese, we use the head rules complied by \newcite{ding2005machine} \footnote{\url{http://stp.lingfil.uu.se/~nivre/research/chn_headrules.txt}}. Since the RedShift implementation of the Zhang-Nivre Parser have not had a Chinese
support yet, we simply train our d-parser using TurboParser second order model \footnote{\newcite{martins2013turning} shows that with third
order features, the UAS of parsing Chinese actually goes down a little bit.} trained with projective constraints.

The result in Table \ref{tab:acc} suggests that, even with out doing any language specific changes in the feature system (e.g. using the last character of a Chinese word as features), we still achieve competitive parsing accuracy. Due to the time constraints, we leave explorations like adopting better d-parse systems such as ZPar \cite{zhang2011syntactic} and feature engineering a future work.
...
\begin{table}
  ...
  \caption{Error analysis of binary CFG rules. Rules used are split into classes based on 
    ...
    ``Total'' is the relative size of the class \nascomment{a
      count might be more interpretable}. ``Acc.'' is the accuracy of span nonterminal identification.
  }
\end{table}
...
\nascomment{no need to reiterate the results presented earlier, I
  commented them out}
