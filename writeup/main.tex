...
utilizing off-the-shelf d-parsers for constructing phrase-structure
...
We instead, consider the same objective, but constrain the c-parses to
...
The running time of the final algorithm reduces to
$O(\sum_{h} |\LeftS(h)||\RightS(h)||\rules|)$. While the terms
$|\LeftS(h)|$ and $|\RightS(h)|$ could in theory make the run-time
...
running-time in practice as we will show in
...
Method 1 utilizes the part-of-speech tag of $x_h$, $\TagFN{h}$, to
...
An important line of previous work also utilizes dependency parsers to
...
a structured prediction to produce the final c-parse.
