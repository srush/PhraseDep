...
public class PhraseStructureTreeConverter {
	...
	public static void ExtractRules(ArrayList<Tree> treeList) throws Exception {
		int index = 0;
		...
		for (PTBRule r : ruleSet) {
			if (r.getRhs().size() > 2) {
				...
				// As Sasha suggested, we will add the minimal number of
				// left/right
	...
	public static void ConvertPennTreeToDep(ArrayList<Tree> treeList) throws Exception {
		...
		for (Tree t : treeList) {
			...
			for (Tree leaf : leaves) {
				...
				lw.writeln(index + "\t" + wordForm + "\t_\t" + pos + "\t" + pos
						+ "\t_\t" + parent + "\t_\t_");
			}
			lw.writeln();
	...
	public static void printLexicalizedTree(ArrayList<Tree> treeList) throws Exception {
	...
	private static Tree getLexicalizedTree(Tree ct) {
		...
		for (Tree st : tlist) {
			if (st.isLeaf()) {
				st.setValue(getWordFromString(st.nodeString()) + "^"
						+ getIndexFromString(st.nodeString()));
				continue;
			}
			st.setValue(st.nodeString() + "^"
					+ getIndexFromString(headMap.get(st)));
		}
		return ct;
	}
