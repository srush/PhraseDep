...
the second stage refines it \cite{petrov2007improved}. \nascomment{I
  think Charniak did coarse-to-fine before that, should cite that one too}
...
semisupervised training (Section~\ref{sec:exper}).
...
further assumptions on the scoring function.  \nascomment{should cite
  Eisner and Satta '99 who show improved asymptotics under some
  assumptions; reviewers will complain if we don't}
...
To make this prediction, we estimate a distribution \[p(\textrm{side} \mid \TagFN{x_h}, \TagFN{x_l}, \TagFN{x_r})\]
where $x_l$ is the next left dependent, $x_r$ is the next right
dependent, and $\TagFN{}$ is the part-of-speech tag of the
word. \nascomment{unclear how this gets used to prune.  need to explain. } This
reduces the  effect of the $n^2$ at runtime. 
...
adapted from existing \nascomment{lexicalized?} c-parsers. However, the existing structure of
...
\nascomment{I am confused by the max with 0.  I don't think that's
  standard for structured hinge}

\noindent where we define $\ell$ as
\nascomment{added negation to first term!}
...
of rule productions \nascomment{coupled with $i,j,k$ positions?  would
  be strange and difficult to do what you said!}. \lpkcomment{yes, coupled with i,j,k}
...
\S 2--21, development on \S 22 \nascomment{clarify what gets tuned here}, and testing on \S 23.
...
is done using the RedShift
implementation\footnote{\url{https://github.com/syllog1sm/redshift}} of the
Zhang-Nivre Parser \cite{zhang2011transition},
 trained to follow the conventions of Collins head rules 
\cite{collins2003head}. \nascomment{need to explain beam size here,
  since it comes up later!}


Prior to training the d-parser, the training sections are
automatically processed  using 10-fold jackknifing \cite{collins2000} for both dependency and phrase structure trees. \newcite{zhu2013fast} found this
...
standard setup. We report labeled \nascomment{yes?} $F_1$ scores as well as recall and
...
independently trained, but complete end-to-end c-parser
\nascomment{say explicitly that runtime includes d-parsing!}.  This setup
