...
First, we observe that in training the part-of-speech of the head word
$x_h$ greatly restricts the possible rules
$\Rule{A}{\beta_1}{\beta_2}$ available.  To exploit this property we
build tables $\rules_{\TagFN{x_h}}$ where $\TagFN{}$ is the
part-of-speech tag of word $x_h$ and further limit the search to
rules seen in training for each head tag. This further reduces the run-time of the
algorithm to $O(\sum_{h} (L(h) \times R(h) \times
|\rules_{\TagFN{x_h}}|))$.
...
where $x_l$ is the next left dependent and $x_r$ is the next right
dependent. If
...
\begin{table}
  ...
  \begin{tabular}{|l|crr|}
    ...
    \textsc{Prune1}   & $\sum_h L(h) R(h) |\rules|_{t(h)}$ &  336.0 & 92.5    \\
  ...
  \caption{Comparison of three parsing setups: 
    \textsc{Full$^*$} is the standard lexicalized grammar,
    but limited to only sentences less than 20 words,
    \textsc{Dep} restricts c-parses to be consistent with a
    predicted dependency skeleton, and \textsc{Pruned1}, \textsc{Pruned2}, and \textsc{Pruned1+2}
    are the pruning methods described in
