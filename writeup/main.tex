...
\item For a word $x_h$ with $L$ dependents to its left and $R$ dependents to 
its right, there are $L\times R$ possible orders to select dependents in the 
algorithm, since at any point a rule can take a left or right dependent. 
...
\item
$\Rule{\bar{A}}{A^{|*}}{\beta_i}$ for $k < i < m$

\item
$\Rule{\bar{A}} {\beta_i}{\bar{A}^{*}} $ for $1< i < k$
\end{itemize}
